                              ...
                              bfFunc* bfFuncTbl, bpFunc* bpFuncTbl)
{
        ...
        !nodeDeleteFunc || !name)
    {
        fprintf (
            stderr,
            "Improper use of registerNodeClass(), all args must be filled\n");
        return -1;
    }
    ...
    if (lsddb_addNodeClass (&tempClass, key->dbId, name, desc, classIdx) < 0)
    {
        fprintf (
            stderr,
            "Error while running DB and array portion of registerNodeClass()\n");
        return -1;
    }
                             ...
                             const char* name, const char* desc)
{
    ...
    if (lsddb_addDataType (&typeId, key->dbId, name, desc) < 0)
    {
        fprintf (
            stderr,
            "Error while running DB and Array portion of registerDataType()\n");
        return -1;
    }
