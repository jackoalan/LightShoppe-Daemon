...
int bufferUnivs(){
	...
	for(i=0;i<=chanArr->maxIdx;++i){
		...
		if(chan && chan->output && chan->output->typeId==rgbType){
			...
			if(chan->rAddr.b16)
				chan->rAddr.univ->buffer[chan->rAddr.addr + 1] = (rVal)&0xff;
			...
			if(!chan->single){
				...
				if(chan->gAddr.b16)
					chan->gAddr.univ->buffer[chan->gAddr.addr + 1] = (gVal)&0xff;
				
				// Blue
				bVal = lround(rgb->b*0xffff);
				chan->bAddr.univ->buffer[chan->bAddr.addr] = bVal>>8;
				if(chan->bAddr.b16)
					chan->bAddr.univ->buffer[chan->bAddr.addr + 1] = (bVal)&0xff;
				
			}
		}
		else{
			//fprintf(stderr,"Channel's Output isn't connected or isn't standard RGB\n");
