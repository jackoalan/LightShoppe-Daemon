...
void
destruct_SceneNodeOutput (void* nodeOutput);
...
void*
node_bufferOutput (struct LSD_SceneNodeOutput* output);
...
void
destruct_SceneNodeInst (void* nodeInst);


/**
  * Notes on SceneNodeInstArr:
  *
  * Insertion and removal of scene nodes is governed by the
  *Plugin API controller and will follow some simple
  *management procedures to prevent fragmentation of this
  *array.
  *
  * First, this array is reallocated when five (5) free
  *entry slots are left. It is reallocated to the next
  *multiple.
  *
  * When entries are freed and their members are
  *deallocated, the index of the array where the removal
  *occured is logged in a mark-sweep table of the database
  *(with set,discover,and unset functions)
  */
...
/**
  * Discover open space using marksweep table (or use
  *instCount as index) to grant instance a spot in the array
  */
int
instarr_addNodeInst (int dbId, int* arrIdxBind);


int
instarr_removeNodeInst (int dbId, int* freedIdxBindj);


/**
  * SceneNodeInputArr and SceneNodeOutputArr behave in a
  *similar manner as SceneNodeInstArr
  */
