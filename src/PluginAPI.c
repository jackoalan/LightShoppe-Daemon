...
static const char LOG_COMP[] = "PluginAPI.c";
...
int
plugininit_registerNodeClass (struct LSD_ScenePlugin const* key,
                              ...
                              bfFunc* bfFuncTbl, bpFunc* bpFuncTbl)
{
    ...
    if (!key || !nodeMakeFunc || !nodeRestoreFunc || !nodeCleanFunc ||
        !nodeDeleteFunc || !name)
    {
        doLog (ERROR, LOG_COMP, _("Improper use of registerNodeClass(): all args must be filled."));
        return -1;
    }
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while running DB and array portion of registerNodeClass()."));
        return -1;
    }
                             ...
                             const char* name, const char* desc)
{
    ...
    if (!key || !name)
    {
        doLog (ERROR, LOG_COMP, _("Improper use of registerDataType()."));
        return -1;
    }
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while running DB and Array portion of registerDataType()."));
        return -1;
    }
                    ...
                    void** dataBind)
{
    ...
    if (lsddb_resolveInstFromId (&inst, nodeId, dataBind) == 0)
    {
        if (inst->nodeClass->plugin == key)
            return inst;
        doLog (ERROR, LOG_COMP, _("Inst request with id %d failed ownership test."), nodeId);
    }
    return NULL;
}
                        ...
                        const char* coldef)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while creating table in plugin API."));
        return -1;
    }
                        ...
                        const char* coldef)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while creating table index in plugin API."));
        return -1;
    }
                     ...
                     const char* wherePortion)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while prepping SELECT in plugin API."));
        return -1;
    }
                     ...
                     const char* valuesPortion)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while prepping INSERT in plugin API."));
        return -1;
    }
                     ...
                     const char* wherePortion)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while prepping UPDATE in plugin API."));
        return -1;
    }
                     ...
                     const char* wherePortion)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Error while prepping DELETE in plugin API."));
        return -1;
    }
...
plugindb_reset (struct LSD_ScenePlugin const* key, unsigned int stmtBinding)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Unable to reset stmt."));
        return -1;
    }
...
plugindb_step (struct LSD_ScenePlugin const* key, unsigned int stmtBinding)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Unable to step stmt."));
        return -1;
    }
                      ...
                      double data)
{
    ...
    int result;
    if (lsddbapi_stmtBindDouble (key, stmtBinding, sqlBinding, data,
                                 &result) < 0)
    {
        doLog (ERROR, LOG_COMP, _("Unable to bind double to stmt."));
        return -1;
    }
                   ...
                   unsigned int sqlBinding, int data)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Unable to bind int to stmt."));
        return -1;
    }
                     ...
                     sqlite3_int64 data)
{
    ...
    int result;
    if (lsddbapi_stmtBindInt64 (key, stmtBinding, sqlBinding, data,
                                &result) < 0)
    {
        doLog (ERROR, LOG_COMP, _("Unable to bind int64 to stmt."));
        return -1;
    }
                    ...
                    unsigned int sqlBinding)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Unable to bind null to stmt."));
        return -1;
    }
...
int
plugindb_bind_text (struct LSD_ScenePlugin const* key, unsigned int stmtBinding,
                    unsigned int sqlBinding, const char* data, int length,
                    void ( *destructor )(void*))
{

    int result;
    if (lsddbapi_stmtBindText (key, stmtBinding, sqlBinding, data, length,
                               destructor, &result) < 0)
    {
        doLog (ERROR, LOG_COMP, _("Unable to bind text to stmt."));
        return -1;
    }
...
int
plugindb_bind_text16 (struct LSD_ScenePlugin const* key,
                      unsigned int stmtBinding,
                      unsigned int sqlBinding,
                      const void* data,
                      int length,
                      void ( *destructor )(void*))
{
    ...
    int result;
    if (lsddbapi_stmtBindText16 (key, stmtBinding, sqlBinding, data, length,
                                 destructor, &result) < 0)
    {
        doLog (ERROR, LOG_COMP, _("Unable to bind text16 to stmt."));
        return -1;
    }
                        ...
                        int colIdx)
{
    ...
    {
        doLog (ERROR, LOG_COMP, _("Unable to get column from stmt."));
        return 0.0;
    }
