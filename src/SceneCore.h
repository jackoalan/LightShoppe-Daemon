...
/**
  * This file provides bindings for initialising and
  *maintaining all necessary arrays and in-memory database
  *to maintain LSD's state. All arrays are allocated once at
  *startup and requiring deallocation and reallocation
  *(restart) to introduce additional entries. An expection
  *to this rule is the SceneNodeInstArr. This array is a
  *dynamic one which always holds a default multiple of 50
  *instance entries. This multiple can be adjusted by the
  *user in LSD's configuration (argument or otherwise).
  */


/**
  * Late Break:
  * This file also contains (following this message) Low
  *Level object structures for Partitions, Channels,
...
int
lsdSceneRevert (const char* dbpath, sqlite3* revertTarget);
