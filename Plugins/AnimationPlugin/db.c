...
/* Attack Decay Stuff */
int
createAttackDecay (int nodeId, int inId)
{
...
int
getAttackDecayVals (int nodeId, double* attackBind, double* decayBind)
{
    ...
    {
        if (attackBind)
            *attackBind = plugindb_column_double (animationPlugin,
                                                  selectAttackDecay,
                                                  0);
        if (decayBind)
            *decayBind = plugindb_column_double (animationPlugin,
                                                 selectAttackDecay,
                                                 1);
        return 0;
    }
...
int
restoreAttackDecay (struct LSD_SceneNodeInst const* inst)
{
    if (!inst)
        return -1;
    ...
    if (plugindb_step (animationPlugin, selectAttackDecay) == SQLITE_ROW)
    {
        castData->attackRate = plugindb_column_double (animationPlugin,
                                                       selectAttackDecay,
                                                       0);
        castData->decayRate = plugindb_column_double (animationPlugin,
                                                      selectAttackDecay,
                                                      1);
...
int
updateAttackDecayAttack (int nodeId, double attackAmt)
{
    ...
    plugindb_bind_double (animationPlugin,
                          updateAttackDecay_attack,
                          2,
                          attackAmt);
    if (plugindb_step (animationPlugin,
                       updateAttackDecay_attack) == SQLITE_DONE)
    {
...
int
updateAttackDecayDecay (int nodeId, double decayAmt)
{
...
int
deleteAttackDecayDo (int nodeId)
{
...
/* Main db init */
int
initAnimDB (struct LSD_ScenePlugin const* plugin)
{
    animationPlugin = plugin;

    /* Attack Decay stuff */
    plugininit_createTable (
        plugin,
        ATTACK_DECAY,
