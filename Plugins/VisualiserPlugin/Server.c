...
void localBufCopy(struct LSD_SceneNodeOutput const * output){
    // Get semaphore
    sem[0].sem_op = 0;
    sem[1].sem_op = 1;
...
void* getHigh(struct LSD_SceneNodeOutput const * output){
    return &(localBuffer[2]);
}

void* getMid(struct LSD_SceneNodeOutput const * output){
    return &(localBuffer[1]);
}

void* getLow(struct LSD_SceneNodeOutput const * output){
...
int makeNode(struct LSD_SceneNodeInst const * inst, void* instData){
...
int restoreNode(struct LSD_SceneNodeInst const * inst, void* instData){return 0;}

void cleanNode(struct LSD_SceneNodeInst const * inst, void* instData){}

void deleteNode(struct LSD_SceneNodeInst const * inst, void* instData){}
...
int semInit(){
    ...
    // Get Shared Memory
    shmid = shmget(shmkey, sizeof(double)*NUM_BANDS, 0666 | IPC_CREAT | IPC_EXCL);
    if(shmid < 0){
...
int visPluginInit(struct LSD_ScenePlugin const * plugin){
    ...
    plugininit_registerNodeClass(plugin,NULL,makeNode,restoreNode,cleanNode,deleteNode,
                                 0,"MPD Visualiser","Desc",0,bfFuncs, bpFuncs);
    
    return 0;
}

void visPluginClean(struct LSD_ScenePlugin const * plugin){
...
void visPluginRPC(cJSON* in, cJSON* out){}
...
/*
 * In order for LightShoppe to properly traverse scopes and connect 
 * to the Plugin HEAD, an accessor function named `getPluginHead` 
 * must be included by the plugin author that returns a reference 
 * to the Plugin HEAD
 */
