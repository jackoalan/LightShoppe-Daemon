...
void pickerBufferOut(struct LSD_SceneNodeOutput const * output){
	// Nothing needs to be done
}

void* pickerPointerOut(struct LSD_SceneNodeOutput const * output){
...
int colourBankNodeMake(struct LSD_SceneNodeInst const * inst, void* instData){
...
int colourBankNodeRestore(struct LSD_SceneNodeInst const * inst, void* instData){
	...
	int i = 0;
	while(plugindb_step(pickerBankPlugin, selectPickerNodeStmt) == SQLITE_ROW && i < cnt){
		
		castData->pickerArr[i].dbId = plugindb_column_int(pickerBankPlugin, 
														  selectPickerNodeStmt,
														  0);
		castData->pickerArr[i].pickerVal.r = plugindb_column_double(pickerBankPlugin, 
																	selectPickerNodeStmt,
																	1);
		castData->pickerArr[i].pickerVal.g = plugindb_column_double(pickerBankPlugin, 
																	selectPickerNodeStmt,
																	2);
		castData->pickerArr[i].pickerVal.b = plugindb_column_double(pickerBankPlugin, 
																	selectPickerNodeStmt,
																	3);
...
void colourBankNodeClean(struct LSD_SceneNodeInst const * inst, void* instData){
	struct PickerInstData* castData = (struct PickerInstData*)instData;
    free(castData->pickerArr);
	printf("Node Clean ran\n");
}

void colourBankNodeDelete(struct LSD_SceneNodeInst const * inst, void* instData){
...
void colourBankRPCHandler(cJSON* in, cJSON* out){
	...
	if(cbMethod && cbMethod->type==cJSON_String){
        ...
        if(strcasecmp(cbMethod->valuestring,"getNodePickers")==0){
			...
			plugindb_reset(pickerBankPlugin, selectPickerNodeStmt);
			plugindb_bind_int(pickerBankPlugin, selectPickerNodeStmt, 1, nodeId->valueint);
			while(plugindb_step(pickerBankPlugin, selectPickerNodeStmt) == SQLITE_ROW){
				int pickerId = plugindb_column_int(pickerBankPlugin,selectPickerNodeStmt,0);
				double lastR = plugindb_column_double(pickerBankPlugin,selectPickerNodeStmt,1);
				double lastG = plugindb_column_double(pickerBankPlugin,selectPickerNodeStmt,2);
				double lastB = plugindb_column_double(pickerBankPlugin,selectPickerNodeStmt,3);
			...
			if(!rVal || rVal->type != cJSON_Number ||
			   ...
			   !bVal || bVal->type != cJSON_Number){
				cJSON_AddStringToObject(out,"error","Invalid colour provided");
				return;
			}
			
			plugindb_reset(pickerBankPlugin, updatePickerStmt);
			plugindb_bind_int(pickerBankPlugin, updatePickerStmt, 1, pickerId->valueint);
			plugindb_bind_double(pickerBankPlugin, updatePickerStmt, 2, rVal->valuedouble);
			plugindb_bind_double(pickerBankPlugin, updatePickerStmt, 3, gVal->valuedouble);
			plugindb_bind_double(pickerBankPlugin, updatePickerStmt, 4, bVal->valuedouble);

			if(plugindb_step(pickerBankPlugin, updatePickerStmt) != SQLITE_DONE){
				cJSON_AddStringToObject(out,"error","Error updating picker");
				return;
			}
			
			// Update buffered values
			inst = plugin_getInstById(pickerBankPlugin,nodeId->valueint,&instData);
			if(instData){
				struct PickerInstData* castData = (struct PickerInstData*)instData;
				int i;
			...
			if(inst){
				...
				if(plugininst_addInstOutput(inst,rgbTypeId,"Output",0,0,&outId)<0){
					cJSON_AddStringToObject(out,"error","Couldn't add output to node");
					return;
				}
				...
				cJSON_AddStringToObject(out,"error","Couldn't get inst by id");
			}
			
			plugindb_reset(pickerBankPlugin, insertPickerStmt);
			plugindb_bind_int(pickerBankPlugin, insertPickerStmt, 1, outId);
			plugindb_bind_int(pickerBankPlugin, insertPickerStmt, 2, nodeId->valueint);
			
			if(plugindb_step(pickerBankPlugin, insertPickerStmt) != SQLITE_DONE){
			...
			if(inst){
				...
				if(plugininst_removeInstOutput(inst,pickerId->valueint)<0){
					cJSON_AddStringToObject(out,"error","Couldn't remove output from node");
					return;
				}
			...
			plugindb_bind_int(pickerBankPlugin, deletePickerStmt, 1, pickerId->valueint);
			
			if(plugindb_step(pickerBankPlugin, deletePickerStmt) != SQLITE_DONE){
...
int colourBankPluginInit(struct LSD_ScenePlugin const * plugin){
    ...
    if(plugininit_registerNodeClass(plugin,&colourBankClass,colourBankNodeMake,colourBankNodeRestore,
									colourBankNodeClean,colourBankNodeDelete,sizeof(struct PickerInstData),
									"Colour Bank","Colour Bank Class",0,bfFuncs,bpFuncs)<0){
	...
	if(plugininit_createTable(plugin,"picker",
							  ...
							  "lastG REAL DEFAULT 0, lastB REAL DEFAULT 0")<0){
		fprintf(stderr,"Unable to register db tables for colour bank plugin\n");
		return -1;
	}
	...
	if(plugindb_prepUpdate(plugin, &updatePickerStmt,
						   "picker", "lastR=?2,lastG=?3,lastB=?4", "id=?1")<0){
...
void colourBankPluginClean(struct LSD_ScenePlugin const * plugin){
    // Nothing to be done here
}
