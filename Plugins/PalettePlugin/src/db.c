 ...
 **palette sampler plugin. It's init function must be ran at
 **plugin init.
 */
...
/* Forward Decls */
int
paletteDBRemakeSamplers (int nodeId,
                         int numOuts,
...
int
getPaletteSwatches (cJSON* target, int paletteId)
{
    ...
    while (plugindb_step (palettePlugin, selectSwatchStmt) == SQLITE_ROW)
    {
        ...
        cJSON_AddNumberToObject (swatchObj, "swatchId",
                                 plugindb_column_int (palettePlugin,
                                                      selectSwatchStmt, 0));
        cJSON_AddNumberToObject (swatchObj, "rVal",
                                 plugindb_column_double (palettePlugin,
                                                         selectSwatchStmt, 1));
...
paletteDBGetSampler (cJSON* target, int nodeId)
{
    ...
    if (plugindb_step (palettePlugin, selectNodeSettingStmt) == SQLITE_ROW)
    {
        cJSON_AddNumberToObject (settingObj, "selMode",
                                 plugindb_column_int (palettePlugin,
                                                      selectNodeSettingStmt, 0));
        cJSON_AddNumberToObject (settingObj, "manualPaletteId",
                                 plugindb_column_int (palettePlugin,
                                                      selectNodeSettingStmt, 1));
        cJSON_AddNumberToObject (settingObj, "numOuts",
                                 plugindb_column_int (palettePlugin,
                                                      selectNodeSettingStmt, 2));
        sampleMode = plugindb_column_int (palettePlugin,
                                          selectNodeSettingStmt,
                                          3);
    ...
    while (plugindb_step (palettePlugin, selectPaletteStmt) == SQLITE_ROW)
    {
        ...
        int paletteId = plugindb_column_int (palettePlugin,
                                             selectPaletteStmt,
                                             0);
        cJSON_AddNumberToObject (paletteObj, "paletteId", paletteId);
        cJSON_AddStringToObject (paletteObj, "paletteName",
                                 (const char*)plugindb_column_text (
                                     palettePlugin, selectPaletteStmt, 1));
    ...
    {
        cJSON_AddStringToObject (target,
                                 "error",
                                 "error getting current palette");
        return -1;
    }
    ...
    if (sampleMode == 0)
        ...
        for (i = 0; i < instData->numOuts; ++i)
        {
            ...
            struct PaletteSamplerOutputData* outData =
                &( instData->outDataArr[i] );
...
paletteDBSetSelMode (int nodeId, enum SourceModeEnum mode)
{
    ...
    if (plugindb_step (palettePlugin,
                       updateNodeSettingSelModeStmt) == SQLITE_DONE)
        return 0;
    return -1;

    /* Get node */
    struct PaletteSamplerInstData* instData = NULL;
    struct LSD_SceneNodeInst const* inst = plugin_getInstById (
        palettePlugin,
        nodeId,
        (void**)&
        instData);
...
paletteDBUpdateSwatchColour (int swatchId, cJSON* colour)
{
    ...
    plugindb_bind_double (palettePlugin,
                          updateSwatchColourStmt,
                          2,
                          rVal->valuedouble);
    plugindb_bind_double (palettePlugin,
                          updateSwatchColourStmt,
...
int
paletteDBUpdateSwatchPos (int swatchId, int posIdx)
{
...
paletteDBLoadSwatchData (struct LSD_SceneNodeInst const* inst, int paletteId)
{
    ...
    struct PaletteSamplerInstData* instData =
        (struct PaletteSamplerInstData*)inst->data;
    ...
    struct RGB_TYPE* swatchArr = malloc (
        sizeof( struct RGB_TYPE ) * swatchCount);
    if (!swatchArr)
        return -1;
    ...
    plugindb_bind_int (palettePlugin, selectSwatchStmt, 1, paletteId);
    int i = 0;
    while (plugindb_step (palettePlugin,
                          selectSwatchStmt) == SQLITE_ROW && i < swatchCount)
    {
        instData->swatchArr[i].r = plugindb_column_double (palettePlugin,
                                                           selectSwatchStmt,
                                                           1);
        instData->swatchArr[i].g = plugindb_column_double (palettePlugin,
                                                           selectSwatchStmt,
                                                           2);
        instData->swatchArr[i].b = plugindb_column_double (palettePlugin,
                                                           selectSwatchStmt,
                                                           3);
        ++i;
    }
...
paletteDBActivatePalette (int nodeId, int paletteId)
{
    ...
    plugindb_bind_int (palettePlugin,
                       updateNodeSettingManualPaletteStmt,
                       1,
                       nodeId);
    plugindb_bind_int (palettePlugin,
                       updateNodeSettingManualPaletteStmt,
                       ...
                       paletteId);
    if (plugindb_step (palettePlugin,
                       updateNodeSettingManualPaletteStmt) == SQLITE_DONE)
    {
                         ...
                         struct LSD_SceneNodeInst const* inst)
{
    ...
    {
        int inId = plugindb_column_int (palettePlugin,
                                        selectSampleStopInStmt,
                                        0);
    ...
    if (plugindb_step (palettePlugin, selectNodeSettingStmt) == SQLITE_ROW)
    {
        int sampleMode = plugindb_column_int (palettePlugin,
                                              selectNodeSettingStmt,
                                              3);

        /* if manual (0), layout samplers evenly and
         * register in DB */
        if (sampleMode == 0)
        {
        ...
        /* if param (1), create that many sampler inputs and
         * register in DB */
...
int
paletteDBSetNumOut (int nodeId, int numOuts)
{
    ...
    if (plugindb_step (palettePlugin,
                       updateNodeSettingNumOutsStmt) == SQLITE_DONE)
    {
        ...
        /* Select each output from database and request its
         * removal */
...
paletteDBSampleMode (int nodeId, enum SourceModeEnum mode)
{
    ...
    plugindb_bind_int (palettePlugin,
                       updateNodeSettingSampleModeStmt,
                       1,
                       nodeId);
    plugindb_bind_int (palettePlugin, updateNodeSettingSampleModeStmt, 2, mode);
    if (plugindb_step (palettePlugin,
                       updateNodeSettingSampleModeStmt) == SQLITE_DONE)
    {
...
paletteDBManualSampleStopPos (int nodeId, int stopId, double pos)
{
    ...
    if (plugindb_step (palettePlugin, updateManualStopPosStmt) == SQLITE_DONE)
    {
        ...
        /* Find the associated stop struct and set its
         * position there */
        struct PaletteSamplerInstData* instData = NULL;
        plugin_getInstById (palettePlugin, nodeId, (void**)&instData);
...
paletteDBRepeatMode (int nodeId, enum RepeatMode mode)
{
    ...
    plugindb_bind_int (palettePlugin,
                       updateNodeSettingRepeatModeStmt,
                       1,
                       nodeId);
    plugindb_bind_int (palettePlugin, updateNodeSettingRepeatModeStmt, 2, mode);
    if (plugindb_step (palettePlugin,
                       updateNodeSettingRepeatModeStmt) == SQLITE_DONE)
    {
...
restorePaletteInst (struct LSD_SceneNodeInst const* inst)
{
    ...
    {
        /* Settings */
        instData->selMode = plugindb_column_int (palettePlugin,
                                                 selectNodeSettingStmt,
                                                 0);
        instData->curPaletteId = plugindb_column_int (palettePlugin,
                                                      selectNodeSettingStmt,
                                                      1);
        instData->numOuts = plugindb_column_int (palettePlugin,
                                                 selectNodeSettingStmt,
                                                 2);
        instData->sampleMode = plugindb_column_int (palettePlugin,
                                                    selectNodeSettingStmt,
                                                    3);
        instData->paramSampleRepeatMode = plugindb_column_int (
            palettePlugin,
            selectNodeSettingStmt,
            4);
        ...
        struct PaletteSamplerOutputData* outDataArr =
            malloc (sizeof( struct PaletteSamplerOutputData ) *
                    instData->numOuts);
        ...
        while (plugindb_step (palettePlugin,
                              selectRGBOutStmt) == SQLITE_ROW && i <
               instData->numOuts)
        {
            instData->outDataArr[i].outId = plugindb_column_int (
                palettePlugin,
                selectRGBOutStmt,
                0);
            instData->outDataArr[i].outIdx = plugindb_column_int (
                palettePlugin,
                selectRGBOutStmt,
                1);
            ++i;
        ...
        if (instData->selMode == MANUAL)
        {
            ...
            plugindb_bind_int (palettePlugin,
                               selectSwatchStmt,
                               1,
                               instData->curPaletteId);
            int swatchCount = 0;
            while (plugindb_step (palettePlugin,
                                  selectSwatchStmt) == SQLITE_ROW)
            ...
            plugindb_bind_int (palettePlugin,
                               ...
                               instData->curPaletteId);
            while (plugindb_step (palettePlugin,
                                  selectSwatchStmt) == SQLITE_ROW && i <
                   swatchCount)
            {
                instData->swatchArr[i].r = plugindb_column_double (
                    palettePlugin,
                    selectSwatchStmt,
                    1);
                instData->swatchArr[i].g = plugindb_column_double (
                    palettePlugin,
                    selectSwatchStmt,
                    2);
                instData->swatchArr[i].b = plugindb_column_double (
                    palettePlugin,
                    selectSwatchStmt,
                    3);
                ++i;
        ...
        else if (instData->selMode == PARAM)
        {
            ...
            /* Remove curPalette Reference and wait for
             * buffering */
        ...
        if (instData->sampleMode == PARAM)
        {
            struct LSD_SceneNodeInput const** sampleStopInArr =
                malloc (
                    sizeof( struct LSD_SceneNodeInput* ) * instData->numOuts);
            if (!sampleStopInArr)
                return -1;
            ...
            int i = 0;
            while (plugindb_step (palettePlugin,
                                  selectSampleStopInStmt) == SQLITE_ROW && i <
                   instData->numOuts)
            {
                plugininst_getInputStruct (inst,
                                           &( instData->sampleStopInArr[i] ),
                                           plugindb_column_int (palettePlugin,
                                                                selectSampleStopInStmt,
                                                                0));
                ++i;
        ...
        else if (instData->sampleMode == MANUAL)
        {
            ...
            plugindb_bind_int (palettePlugin,
                               selectManualStopStmt,
                               1,
                               inst->dbId);
            int i = 0;
            while (plugindb_step (palettePlugin,
                                  selectManualStopStmt) == SQLITE_ROW)
            {
                instData->outDataArr[i].samplePos = plugindb_column_double (
                    palettePlugin,
                    selectManualStopStmt,
                    2);
                ++i;
...
cleanPaletteInst (struct LSD_SceneNodeInst const* inst)
{
    ...
    struct PaletteSamplerInstData* castData =
        (struct PaletteSamplerInstData*)inst->data;
...
/* This table is emptied for an instance when switched to
 * param mode */
/* and repopulated as a default linear spread when switched
 * to manual */
...
paletteSamplerDBInit (struct LSD_ScenePlugin const* plugin)
{
    ...
    plugindb_prepInsert (
        palettePlugin,
        &insertNodeSettingStmt,
        NODE_SETTING,
        "nodeId,selMode,manualPaletteId,numOuts,sampleMode,paramSampleRepeatMode",
        "?1,0,0,0,0,0");
    plugindb_prepUpdate (palettePlugin,
                         &updateNodeSettingSelModeStmt,
                         NODE_SETTING,
                         "selMode=?2",
                         "nodeId=?1");
    plugindb_prepUpdate (palettePlugin,
    ...
    plugininit_createIndex (palettePlugin,
                            "SwatchIndex",
                            SWATCH,
                            "parentPalette,posIdx");
