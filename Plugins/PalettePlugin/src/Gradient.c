...
int
paletteGradientClamp (struct RGB_TYPE* target, int swatchCount,
                      struct RGB_TYPE* swatchArr, double samplePos)
{
    ...
    /* Determine which two swatches the position is in
     * between */
    ...
    for (i = 1; i < swatchCount; ++i)
    {
        ...
        if (swaB >= samplePos)
        {
            /* i is the latter swatch, scale coordinates to
             * focus on area between swatches */
            innerPos = ( samplePos - swaA ) / interval;

            /* Crossfade between swatch i-1 and i using
             * innerPos as blend factor */
            target->r =
                ( swatchArr[i -
                            1].r *
                  ( 1.0 - innerPos )) + ( swatchArr[i].r * innerPos );
            target->g =
                ( swatchArr[i -
                            1].g *
                  ( 1.0 - innerPos )) + ( swatchArr[i].g * innerPos );
            target->b =
                ( swatchArr[i -
                            1].b *
                  ( 1.0 - innerPos )) + ( swatchArr[i].b * innerPos );
...
int
paletteGradientBlackout (struct RGB_TYPE* target, int swatchCount,
                         struct RGB_TYPE* swatchArr, double samplePos)
{
...
int
paletteGradientLoop (struct RGB_TYPE* target, int swatchCount,
                     struct RGB_TYPE* swatchArr, double samplePos)
{
    ...
    if (cyclePos > 1.0)
    {
        ...
        target->r =
            ( swatchArr[swatchCount -
                        1].r * ( 1.0 - xFac )) + ( swatchArr[0].r * xFac );
        target->g =
            ( swatchArr[swatchCount -
                        1].g * ( 1.0 - xFac )) + ( swatchArr[0].g * xFac );
        target->b =
            ( swatchArr[swatchCount -
                        1].b * ( 1.0 - xFac )) + ( swatchArr[0].b * xFac );
        return 0;
    }
    else
    {
...
int
paletteGradientOut (struct LSD_SceneNodeInst const* inst, int outId)
{
    int i;
    ...
    /* Get palette from input if parametric palette
     * selection is active */
    ...
    /* Get sampler positions from inputs if parametric
     * sampling is active */
    if (instData->sampleMode == PARAM)
    {
        for (i = 0; i < instData->numOuts; ++i)
            if (instData->outDataArr[i].outId == outId)
            {
                ...
                double* samplePos = (double*)node_bufferOutput (
                    instData->sampleStopInArr[i]->connection);
                if (samplePos)
    ...
    for (i = 0; i < instData->numOuts; ++i)
        ...
        if (instData->outDataArr[i].outId == outId)
        {
            ...
            if (instData->sampleMode == MANUAL ||
                instData->paramSampleRepeatMode == CLAMP)
                paletteGradientClamp (&( instData->outDataArr[i].outVal ),
                                      instData->swatchCount,
                                      instData->swatchArr,
                                      instData->outDataArr[i].samplePos);
            else if (instData->paramSampleRepeatMode == AUTO_LOOP)
                paletteGradientLoop (&( instData->outDataArr[i].outVal ),
                                     instData->swatchCount,
                                     instData->swatchArr,
                                     instData->outDataArr[i].samplePos);
            else if (instData->paramSampleRepeatMode == AUTO_BLACKOUT)
                paletteGradientBlackout (&( instData->outDataArr[i].outVal ),
                                         instData->swatchCount,
                                         instData->swatchArr,
                                         instData->outDataArr[i].samplePos);
