...
/*
 ** This file is implements the Palette Sampler plugin. It
 **is designed to be a flexible way of maintaining a
 **database of colour palettes and sampling them with an
 **arbitraty number of outputs.
 **
 ** The basic principle involves two main parts:
 **
 ** 1) A database to record palette entries and swatches
 **(individual colours bound to the palette object.
 **
 ** 2) A gradient generator to take any sized palette and
 **evenly distribute its colours across a common domain
 **ranged [0,1]
 **
 ** The palette database is edited in a similar manner to
 **LightShoppe's low-level patch. Add and remove buttons
 **allow the insertion and
 ** removal of palettes and underneath that is the list of
 **swatches bound to the palette.
 **
 ** All instances of the palette sampler have access to the
 **same common database, so palettes can be used in multiple
 **places at once with seperate sampling characteristics.
 **
 ** Each node instance has a setting to determine the
 **palette selection method in manual mode, a 'use' button
 **underneath the palette list is enabled. Selecting a
 **palette from the list and activating this button will
 **make that palette the source. In parameter mode, an
 **integer input is added to the node that will be used to
 **select the palette instead.
 **
 ** Each node instance has a #out setting that determines
 **the number of RGB outputs and the number of sample stops
 **the gradient will have
 **
 ** The gradient sampler of a node has two modes of
 **operation: manual and parameterised. Manual mode enables
 **a gradient viewer strip in the node's configuration
 **dialog. The strip has photoshop-style stops, except the
 **stops aren't used to set the gradient's colours. Instead
 **they're used to define points at which to sample the
 **gradient along it's domain. Stops may cross over
 **eachother without incident.
 **
 ** Parameterised mode will add inputs to the node to accept
 **the sample position from an external source as a floating
 **point value. Values [0,1] will behave just like the
 **manual sampler, however, there is another setting enabled
 **by Parameterisation mode that determines the behaviour of
 **other values outside this range.
 ** - Clamp mode is the default and will output a static
 **colour of either end of the gradient
 ** - Auto loop mode will take the domain delta between two
 **swatches in the gradient and insert a special stop at
 **1+(delta). This stop will be coloured the same as the
 **First colour in the gradient, therefore making a seamless
 **loop. Any sampling beyond 1+(delta) will be automatically
 **wrapped to the beginning of the gradient.
 ** - Auto blackout will use the delta as described above to
 **insert a black stop in the gradient. This will give a
 **smooth fade to black at either end of the gradient.
 **
 ** Please note that all modes behave exactly the same in
 **[0,1]
 **
 */
...
#define getPluginHead  PalettePlugin_LTX_getPluginHead
...
void
paletteRgbBuffer (struct LSD_SceneNodeOutput const* output)
{
...
void*
paletteRgbPtr (struct LSD_SceneNodeOutput const* output)
{
    struct PaletteSamplerInstData* instData =
        (struct PaletteSamplerInstData*)output->parentNode->data;
...
int
paletteSamplerMake (struct LSD_SceneNodeInst const* inst, void* instData)
{
...
int
paletteSamplerRestore (struct LSD_SceneNodeInst const* inst, void* instData)
{
...
void
paletteSamplerClean (struct LSD_SceneNodeInst const* inst, void* instData)
{
...
void
paletteSamplerDelete (struct LSD_SceneNodeInst const* inst, void* instData)
{
...
/* RPC handler
  * -getSampler //palette list, settings, active palette
  *colour stops (for SVG rendering)//
  ...
  */
void
paletteRPCHandler (cJSON* in, cJSON* out)
{
    ...
    else if (strcasecmp (paletteMethod->valuestring, "setSamplePos") == 0)
    {
        ...
        paletteDBManualSampleStopPos (nodeId->valueint,
                                      outId->valueint,
                                      pos->valuedouble);
    }
...
int
palettePluginInit (struct LSD_ScenePlugin const* plugin)
{
    ...
    plugininit_registerNodeClass (plugin,
                                  &paletteSamplerClass,
                                  paletteSamplerMake,
                                  paletteSamplerRestore,
                                  paletteSamplerClean,
                                  paletteSamplerDelete,
                                  sizeof( struct PaletteSamplerInstData ),
                                  "Palette Sampler",
                                  "Desc",
                                  0,
                                  bfFuncs,
...
void
palettePluginClean (struct LSD_ScenePlugin const* plugin)
{
}
